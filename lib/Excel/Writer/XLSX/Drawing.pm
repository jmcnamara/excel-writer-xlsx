...
our @ISA     = qw(Excel::Writer::XLSX::Package::XMLwriter Excel::Writer::XLSX::Worksheet);
our $VERSION = '0.47';
...
# _add_drawing_object()
#
# Add a chart or image or shape sub object to the drawing.
#
...
sub _write_two_cell_anchor {
    ...
    my $self            = shift;
    my $index           = shift;
    my $type            = shift;					# 1 Graphic frame, 2 pic, 3 shape
    my $col_from        = shift;
    my $row_from        = shift;
    ...
    if ( $type == 1 ) {
        ...
        $self->_write_sp( $index, $col_absolute, $row_absolute, $width, $height, $shape);
    }
...
sub _write_sp {
    ...
    if ($shape->{connect} ) {
        ...
        if ($shape->{text}) {
            $self->_write_txBody( $col_absolute, $row_absolute, $width, $height, $shape );
        }
...
sub _write_nv_cxn_sp_pr {
    ...
    my $shape_name = $shape->{name} // join(' ', $shape->{type}, $index);
...
sub _write_nv_sp_pr {
    ...
    my $shape_name = join(' ', $shape->{type}, $index);
    $self->_write_c_nv_pr( $shape->{id}, $shape_name );
...
sub _write_pic {
    ...
    # Write the xdr:spPr element.
    my $shape = {type => 'rect'};       # Pictures are rectangle shapes by default
...
# _write_xdr_sp_pr()
#
# Write the <xdr:spPr> element, specifically for shapes
#
...
sub _write_a_xfrm {
    ...
    my @attributes = ();
    my $rot = ($shape->{rot} // 0) * 60000;
    push(@attributes, ('rot' => "$rot")) if $rot;
...
sub _write_a_av_lst {
    my $self = shift;
    my $shape = shift // {};
    my $adjustments = $shape->{adjustments} // [];
    
    if ( @{$adjustments} ) {
        $self->{_writer}->startTag( 'a:avLst');
        while( my ($s, $sv) = each @{$adjustments} ) {
            # Only connectors have multiple adjustments
            my $ss = $shape->{connect} ? ($s+1) : '';
...
sub _write_a_ln {
    ...
    my $w = $shape->{line_weight} // 1;
    my @attributes = ('w' => $w * 9525);
    $self->{_writer}->startTag( 'a:ln', @attributes);

    my $ln = $shape->{line} // '000000';
    if ( length($ln) > 1 ) {
        # Write the a:solidFill element.
        $self->_write_a_solid_fill( $ln );
    } else {
        $self->{_writer}->emptyTag( 'a:noFill' );
    }
...
sub _write_txBody {
    ...
    my $rot = ($shape->{format}->{_rotation} // 0) * 60000;
    @attributes = (algn=>$shape->{align}, rtl=>$rot);
    ...
    my $sz = ($shape->{format}->{_size} // 8) * 100;
    my $b  =  $shape->{format}->{_bold} // 0;
    my $i  =  $shape->{format}->{_italic} // 0;
    my $u = ($shape->{format}->{_underline} // 0) ? 'sng' : 'none';
    my $so = ($shape->{format}->{_font_strikeout} // 0) ? 'Strike' : 'noStrike';
    ...
    my $color = $shape->{format}->{_color} // 0;
