...
sub _write_chart_type {
    ...
    if ( $args{primary_axes} ) {
        ...
        $self->{_y_axis}->{_major_gridlines} //= 1;
    }
    else {
        ## Reverse X2 and Y2 axes for Bar charts.
        my $old_y2 = $self->{_y2_axis};
        my $old_x2 = $self->{_x2_axis};

        $self->{_y2_axis} = $old_x2;
        $self->{_x2_axis} = $old_y2;
    }
